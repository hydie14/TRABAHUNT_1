...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_booking'])) {
    ...
    if ($existing_booking->num_rows > 0) {
        ...
        if ($stmt_insert->execute()) {
            $request_id = $stmt_insert->insert_id;

            // Notify Service Provider
            $emp_q = $conn->prepare("SELECT company_name FROM employers WHERE employer_id = ?");
            $emp_q->bind_param("i", $user_id);
            $emp_q->execute();
            $emp_data = $emp_q->get_result()->fetch_assoc();
            $emp_q->close();
            $company_name = $emp_data['company_name'] ?? 'A client';

            $notif_title = "New Service Booking Request";
            $notif_msg = htmlspecialchars($company_name) . " has requested your service for '" . htmlspecialchars($service['service_name']) . "'. Please review and respond.";
            $notif_stmt = $conn->prepare("INSERT INTO notifications (user_id, type, reference_id, title, message) VALUES (?, 'service_booking', ?, ?, ?)");
            $notif_stmt->bind_param("iiss", $service['provider_id'], $request_id, $notif_title, $notif_msg);
            $notif_stmt->execute();
            $notif_stmt->close();

            // Redirect to a new 'my_service_bookings.php' page for employers
            $_SESSION['booking_success'] = "Your request for '" . htmlspecialchars($service['service_name']) . "' has been sent to the provider!";
            header("Location: my_service_bookings.php");
            exit();
        } else {
...
<head>
    ...
    <title>Book a Service - Employer</title>
    ...
    <aside class="sidebar">
        <div class="sidebar-header">
         ...
         <nav class="nav-menu">
            ...
            <a href="my_service_bookings.php" class="nav-item active">
        ...
        <div class="user-profile" onclick="toggleProfileDropdown()">
            ...
            <div class="avatar"><?php echo strtoupper(substr($user_data['company_name'], 0, 1)); ?></div>
            <div class="user-info">
        ...
        <div class="section">
            ...
            <form method="POST" onsubmit="return validateBookingTime()">
                ...
                <div class="form-group">
                    ...
                    <textarea id="details" name="details" rows="4" placeholder="e.g., 'We need catering for a small office event of 20 people.' or 'Our office air conditioning unit is not cooling.'" required></textarea>
                </div>
                <div class="form-group">
